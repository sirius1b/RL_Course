...
def values(states, actions, nextSR, gamma ):
	n = len(states)
	E = np.zeros((n,n))
	R = np.zeros((n,1))
	for state in states:
		V_d = np.zeros((n,1))
		R_d = np.zeros((n,1))
		E[states.index(state),states.index(state)] = 1
		...
		for act in acts:
			...
			for sr in SR:
				V_dd[states.index(sr[0])] += pSR[SR.index(sr)]
				R_dd += sr[1]*pSR[SR.index(sr)]

			V_d += pi[acts.index(act)]*V_dd
			R_d[states.index(state)] += pi[acts.index(act)]*R_dd

		V_d = V_d.reshape((n,))
		#print(E[states.index(state)].shape,V_d.shape,(E[states.index(state)]- V_d).shape)
		E[states.index(state)] -= gamma*V_d
		R[states.index(state)] = R_d[states.index(state)]
		# for j in range(n):
		# 	E[states.index(state),j] -= gamma*V_d[j]

	v_pi=np.linalg.solve(E,R)
...
values(states,actions,nextSR,gamma=0.9)
